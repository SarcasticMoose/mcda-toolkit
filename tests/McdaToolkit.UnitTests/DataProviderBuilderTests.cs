using McdaToolkit.Data.Builders;
using McdaToolkit.Data.Validation.Abstraction;

namespace McdaToolkit.UnitTests;

public class DataProviderBuilderTests
{
    [Fact]
    public void Build_WithValidInputs_ShouldReturnCorrectMcdaData()
    {
        var matrix = new[,]
        {
            { 3, 6, 4, 20, 2, 30000 },
            { 4, 4, 6, 15, 2.2, 32000 },
            { 6, 5, 9, 18, 3, 32100 },
            { 5, 6, 3, 23, 2.8, 28000 },
            { 4, 8, 7, 30, 1.5, 29000 },
            { 8, 3, 6, 35, 1.9, 27000 },
            { 7, 2, 5, 33, 1.7, 28500 },
            { 3, 8, 3, 34, 1.6, 30500 },
            { 8, 4, 8, 40, 2.5, 33000 },
            { 9, 3, 7, 34, 2, 29800 },
        };
        double[] weights = { 0.1, 0.2, 0.1, 0.2, 0.1, 0.3 };
        int[] types = { 1, 1, 1, -1, -1, -1 };

        var data = DataBuilder
            .Create()
            .AddWeights(weights)
            .AddDecisionCriteria(types)
            .AddDecisionMatrix(matrix)
            .Build();

        Assert.NotNull(data);
        Assert.Equal(weights, data.Weights.Storage.ToArray());
        Assert.Equal(types, data.Types);
        Assert.Equal(matrix, data.Matrix.Storage.ToArray());
    }

    [Fact]
    public void Build_WithoutDecisionMatrix_ShouldThrowArgumentNullException()
    {
        double[] weights = { 0.1, 0.2, 0.1, 0.2, 0.1, 0.3 };
        int[] types = { 1, 1, 1, -1, -1, -1 };

        var ex = Assert.Throws<ValidationException>(() =>
        {
            DataBuilder.Create().AddWeights(weights).AddDecisionCriteria(types).Build();
        });

        Assert.IsType<ValidationException>(ex);
    }
}
